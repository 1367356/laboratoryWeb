$(document).ready(function() {
    CKEDITOR.instances['editor1'].setData($("input[name='edit_content']").val());
    (function() {
		var pid = parseInt($('#nav_pid').text());
		var id = parseInt($('#nav_id').text());
		var type_sel = $('input[name="type"]').val();
		if (pid == 4) {
			$('.for_type').empty();
			var type_li = '';
			switch(id){
			...
			$.each($('.for_type option'),function(key,val){
				var $this = $('.for_type option:eq('+key+')');
				if($this.val() == type_sel){
